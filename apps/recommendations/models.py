...
class SimpleFavorite(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='simple_favorites')
...
class SimpleViewingHistory(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='simple_viewing_history')
    movie_id = models.PositiveIntegerField(help_text="TMDb movie ID")
...
class RecommendationCache(models.Model):
    ...
    scores = models.JSONField(help_text="Recommendation scores for each movie", null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class RecommendationFeedback(models.Model):
    ...
    recommendation_type = models.CharField(max_length=20, choices=RecommendationCache.RECOMMENDATION_TYPES)
    ...
    def __str__(self):
        return f"{self.user.email} - Movie {self.movie_id}: {self.get_feedback_display()}"
...
class RecommendationSettings(models.Model):
    ...
    user = models.OneToOneField(
        User, 
        on_delete=models.CASCADE, 
        related_name='recommendation_settings'
    )
    ...
    min_vote_count = models.IntegerField(
        default=100,
